...
\hline
\rowcolor{yellow!20}
\textbf{Conformidad} & \textcolor{green}{\textbf{✓ Completa}} & 7/7 FEs implementadas con protocolos concretos, interfaces estandarizadas, mapeo verificable \\
\hline
\end{tabular}
}
\end{table}
...
Los estándares IEEE 2030.5, ISO/IEC 30141 e IEC 61850 establecen el marco conceptual y regulatorio para sistemas de Energía Inteligente (\textit{Smart Energy}) interoperables~\cite{IEEERecommendedPractice,tangResearchInteroperabilityIoT,basnetNetworkingPowerGrid2025}. Sin embargo, su implementación práctica requiere una pila de protocolos IoT (\textit{protocol stack}) optimizada para dispositivos restringidos (\textit{constrained}) con recursos limitados de CPU, memoria y energía~\cite{choudharyInternetThingsComprehensive2024}. La siguiente sección describe la pila 6LoWPAN/CoAP/LwM2M que materializa estos requisitos~\cite{shelby6LoWPANWirelessEmbedded2009,haEnablingDynamicLightweight2018}, cubriendo desde la capa física (IEEE 802.15.4) hasta la gestión de dispositivos (LwM2M), proporcionando la base técnica sobre la cual se construye la arquitectura propuesta en el Capítulo 3.

\section{Pila de Protocolos IoT para Energía Inteligente (\textit{Protocol Stack for Smart Energy})}

La pila de protocolos (\textit{stack}) para redes IoT en aplicaciones de Energía Inteligente (\textit{Smart Energy}) integra múltiples capas del modelo OSI, optimizando cada una para operar en entornos con restricciones severas: dispositivos con <256 KB RAM, <1 MB Flash, y operación con batería~\cite{abdulsalamOverviewRecentWireless2024}. La arquitectura completa utiliza IEEE 802.15.4 como base PHY/MAC, 6LoWPAN para compresión de IPv6, y CoAP/LwM2M en capa de aplicación, creando una pila extremo-a-extremo (\textit{end-to-end stack}) eficiente y estandarizada~\cite{shelby6LoWPANWirelessEmbedded2009,shahinzadehSmartHomeConnectivity2024}.
...
\textbf{Motivación:} El stack IPv6 tradicional presenta overhead prohibitivo: header IPv6 (40 bytes, 31.5\% del MTU) + header UDP (8 bytes, 6.3\%) = 48 bytes (37.8\%) dejando solo 79 bytes útiles (62.2\%)~\cite{shelby6LoWPANWirelessEmbedded2009,mamoImplementationStandardized6LoWPAN2015}. Cada retransmisión en mesh consume energía crítica en dispositivos battery-powered, motivando optimización extrema del overhead~\cite{shelby6LoWPANWirelessEmbedded2009}.
...
\small
\centering
\caption{Latencia por hop en Thread mesh con/sin compresión 6LoWPAN~\cite{shelby6LoWPANWirelessEmbedded2009}. Escenario: topología lineal 5 hops, IEEE 802.15.4 @ 250 kbps, canal 15 (2.4 GHz). Reducción latencia total: 71\% (7.7 ms → 2.2 ms) mediante IPHC+NHC.}
...
La compresión 6LoWPAN reduce latencia en topologías mesh multi-hop >70\%~\cite{shelby6LoWPANWirelessEmbedded2009,abood6LoWPANTechnicalFeatures2024}, crítico para Smart Energy con requisitos <100 ms.
...
Soluciones dominantes en mercado industrial: Cisco IR829 (\$2.5-4k, LTE/Wi-Fi/Ethernet, sin Thread/HaLow, arquitectura cerrada); Dell Edge Gateway 3000 Series (\$1.2-2k, x86/containers, 25-40W, sin IEEE 2030.5); MultiTech Conduit (\$400-800, LoRaWAN/LTE, CPU limitada 456 MHz ARM, sin capacidades edge analytics avanzadas)~\cite{boonmeerukCostEffectiveIIoTGateway2024}.
