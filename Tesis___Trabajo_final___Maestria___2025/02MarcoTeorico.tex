...
\item \textbf{Network Layer}: 6LoWPAN (IPv6 over Low-Power Wireless Personal Area Networks) - compresión de headers IPv6, fragmentación, mesh-under routing.
\item \textbf{Transport Layer}: UDP (principalmente), TCP limitado por overhead.
\item \textbf{Application Layer}: CoAP (Constrained Application Protocol), MQTT-SN, LwM2M.
\end{itemize}
...
\begin{table}[h]
\centering
\caption{Ejemplo de Tabla de Routing Thread}
...
\hline
\textbf{Stack routing} & IPv6 6LoWPAN & Propietario (AODV) & Managed Flooding \\
\hline
\textbf{Hop limit} & No limit (típico 3-5) & 30 máximo & 127 máximo \\
\hline
\textbf{Route repair} & Proactive (MLE) & Reactive (AODV RERR) & Flooding redundancy \\
\hline
\textbf{Commissioning} & Out-of-band (PSKd) & Install codes & Provisioning (ECDH) \\
\hline
\textbf{Border Router} & Estándar (OTBR) & Coordinador específico & Proxy nodes \\
\hline
\textbf{Matter compatibility} & Nativo & Requiere bridge & Requiere bridge \\
\hline
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{Compresión IPHC de Header IPv6}
\label{tab:iphc-compression}
\begin{tabular}{|l|c|c|c|}
\hline
\textbf{Campo} & \textbf{Original (bytes)} & \textbf{Comprimido (bytes)} & \textbf{Reducción (\%)} \\
\hline
...
\hline
\textbf{Total IPv6} & \textbf{40} & \textbf{2-7} & \textbf{82.5-95\%} \\
\hline
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{Compresión NHC de Header UDP}
\label{tab:nhc-udp}
\begin{tabular}{|l|c|c|c|}
\hline
...
Checksum & 2 & 0 & 100\% \\
\hline
\textbf{Total UDP} & \textbf{8} & \textbf{1-2} & \textbf{75-87.5\%} \\
\hline
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{Latencia por Hop con/sin Compresión 6LoWPAN}
\label{tab:6lowpan-latency}
\begin{tabular}{|l|c|c|c|}
\hline
\textbf{Escenario} & \textbf{Sin Compresión} & \textbf{Con IPHC+NHC} & \textbf{Reducción} \\
\hline
...
\hline
\textbf{Latencia 5 hops mesh} & \textbf{7.7 ms} & \textbf{2.2 ms} & \textbf{71\%} \\
\hline
\end{tabular}
\end{table}
...
\begin{itemize}
\item \textbf{Arquitectura RESTful}: Métodos GET/POST/PUT/DELETE sobre recursos identificados por URIs (ej. \texttt{coap://sensor01/temp}).
...
\item \textbf{Modos CON/NON}: Confirmable (con ACK) para comandos críticos, Non-Confirmable para telemetría best-effort.
...
\begin{table}[h]
\centering
\caption{Comparación CoAP vs HTTP}
\label{tab:coap-vs-http}
\begin{tabular}{|p{4cm}|p{4.5cm}|p{4.5cm}|}
\hline
\textbf{Característica} & \textbf{CoAP/UDP} & \textbf{HTTP/TCP} \\
\hline
\textbf{Header mínimo} & 4 bytes & 100+ bytes (típico 200-500) \\
\hline
\textbf{Transporte} & UDP (8 bytes) & TCP (20 bytes + handshake) \\
\hline
\textbf{Overhead total} & 12-30 bytes & 120-520 bytes \\
\hline
\textbf{Latencia conexión} & 0 ms (stateless) & 50-150 ms (3-way handshake) \\
\hline
\textbf{Formato} & Binario (parsing rápido) & Texto (parsing lento) \\
\hline
\textbf{Subscripciones} & Observe (push nativo) & Polling o WebSocket \\
\hline
\textbf{Fragmentación} & Block-wise (CoAP-aware) & TCP segmentation (opaco) \\
\hline
\textbf{Multicast} & Sí (UDP nativo) & No (TCP unicast only) \\
\hline
\textbf{Seguridad} & DTLS (menor overhead) & TLS (mayor overhead) \\
\hline
\end{tabular}
\end{table}
...
\textbf{Componentes:}
\begin{itemize}
\item \textbf{LwM2M Client}: Ejecuta en dispositivo IoT (ej. medidor inteligente, sensor). Implementa objetos LwM2M y responde a operaciones del servidor.
...
\begin{table}[h]
\centering
\caption{Objetos LwM2M Relevantes para Smart Energy}
...
\begin{table}[h]
\centering
\caption{Bindings de Transporte LwM2M}
\label{tab:lwm2m-bindings}
\begin{tabular}{|c|p{3.5cm}|p{4cm}|p{4cm}|}
\hline
\textbf{Binding} & \textbf{Transporte} & \textbf{Seguridad} & \textbf{Uso Típico} \\
\hline
...
\begin{table}[h]
\centering
\caption{Overhead de Seguridad LwM2M}
\label{tab:lwm2m-security-overhead}
\begin{tabular}{|l|c|c|c|}
\hline
\textbf{Modo} & \textbf{Handshake Size} & \textbf{Per-Message Overhead} & \textbf{Recomendación} \\
\hline
...
\begin{table}[h]
\centering
\caption{Comparación LwM2M vs Alternativas}
\label{tab:lwm2m-comparison}
\begin{tabular}{|p{3.5cm}|p{4cm}|p{4cm}|p{3cm}|}
\hline
\textbf{Característica} & \textbf{LwM2M 1.2} & \textbf{MQTT + JSON} & \textbf{HTTP REST} \\
\hline
\textbf{Overhead típico} & 20-40 bytes & 100-300 bytes & 200-500 bytes \\
\hline
\textbf{Gestión dispositivos} & Nativa (objects std) & Custom (topics) & Custom (endpoints) \\
\hline
\textbf{Firmware OTA} & Estandarizado (Obj 5) & Custom impl & Custom impl \\
\hline
\textbf{Observe/Subscribe} & Nativo + thresholds & MQTT native & Polling o SSE \\
\hline
\textbf{Seguridad} & DTLS-PSK (ligero) & TLS (pesado) & TLS (pesado) \\
\hline
\textbf{Interoperabilidad} & Multi-vendor (OMA) & Propietario & Propietario \\
\hline
\textbf{Complejidad impl} & Media & Baja & Baja \\
\hline
\end{tabular}
\end{table}
...
\begin{itemize}
\item \textbf{Frecuencia}: Bandas regionales no licenciadas: 902-928 MHz (EE.UU./América), 863-868 MHz (Europa), 755-787 MHz (China, Korea), 917-923.5 MHz (Japón).

\item \textbf{Channel width}: 1, 2, 4, 8, 16 MHz (downclocking de 802.11ac por factor 10×).

\item \textbf{Modulación}: MCS 0-10 (BPSK, QPSK, 16-QAM, 64-QAM, 256-QAM opcional), con LDPC o BCC FEC.

\item \textbf{Alcance}: 1-2 km en exteriores (LOS - Line of Sight), 100-300 m en interiores con penetración superior a 2.4/5 GHz gracias a propagación sub-GHz.

\item \textbf{Throughput}: 150 kbps (MCS 0, 1 MHz BW) hasta 86.7 Mbps (MCS 10, 16 MHz BW, 4 spatial streams - teórico).

\item \textbf{Número de estaciones}: Hasta 8,191 dispositivos por AP mediante hierarchical AID (Association Identifier) con páginas.

\item \textbf{Power save}: Target Wake Time (TWT) permite negociar ventanas de actividad, logrando duty cycles <1\% con años de autonomía en batería.
\end{itemize}
...
\begin{tabular}{|c|c|c|c|c|}
\hline
\textbf{MCS} & \textbf{Modulation} & \textbf{Code Rate} & \textbf{Data Rate (Mbps)} & \textbf{Sensitivity (dBm)} \\
\hline
0 & BPSK & 1/2 & 0.150 & -99 \\
1 & QPSK & 1/2 & 0.300 & -96 \\
2 & QPSK & 3/4 & 0.450 & -94 \\
...
\begin{table}[h]
\centering
\caption{Comparación LTE Cat-M1 vs NB-IoT}
\label{tab:lte-iot-comparison}
\begin{tabular}{|p{4cm}|p{4.5cm}|p{4.5cm}|}
\hline
\textbf{Característica} & \textbf{LTE Cat-M1 (eMTC)} & \textbf{NB-IoT} \\
\hline
\textbf{Bandwidth} & 1.4 MHz (6 PRBs) & 200 kHz (1 PRB o standalone) \\
\hline
\textbf{Peak rate DL/UL} & 1 Mbps / 1 Mbps & 250 kbps / 250 kbps (multi-tone) \\
\hline
\textbf{Latency} & 10-15 ms (connected mode) & 1.6-10 s (idle-to-connected) \\
\hline
\textbf{Mobility} & Full mobility (handover) & Limited (reselection only) \\
\hline
\textbf{Voice support} & VoLTE (half-duplex) & No \\
\hline
\textbf{Power consumption} & PSM: 3 µA, eDRX: 0.2 mA & PSM: 5 µA, eDRX: 0.6 mA \\
\hline
\textbf{MCL (Max Coupling Loss)} & 156 dB & 164 dB \\
\hline
\textbf{Deployment} & In-band LTE & In-band / Guard-band / Standalone \\
\hline
\textbf{Use cases} & Asset tracking, wearables, smart city & Medidores, sensores estáticos \\
\hline
\end{tabular}
\end{table}
...
\subsection{Análisis de Capa de Enlace IEEE 802.15.4}
...
\begin{enumerate}
\item \textbf{Inicialización}: Backoff Exponent $BE = macMinBE$ (típicamente 3), Number of Backoffs $NB = 0$.

\item \textbf{Delay aleatorio}: Esperar $rand(0, 2^{BE}-1) \times aUnitBackoffPeriod$ (320 µs @ 2.4 GHz).

\item \textbf{CCA (Clear Channel Assessment)}: Medir energía RF durante 8 symbol periods. Si channel idle → transmitir. Si busy → incrementar $BE = \min(BE+1, macMaxBE)$ y $NB = NB+1$.

\item \textbf{Retry limit}: Si $NB > macMaxCSMABackoffs$ (típicamente 4) → reportar channel access failure.

\item \textbf{Acknowledgment}: Esperar ACK del receptor durante macAckWaitDuration (54 symbol periods = 864 µs). Si no recibido → retransmitir hasta macMaxFrameRetries (típicamente 3).
\end{enumerate}

Probabilidad de colisión para $n$ nodos concurrentes con misma ventana de contención $W$:

\begin{equation}
P_{collision} = 1 - \left(1 - \frac{1}{W}\right)^{n-1}
\end{equation}

Para $W = 2^{BE} = 8$ y $n=10$ nodos:
\begin{equation}
P_{collision} = 1 - \left(\frac{7}{8}\right)^9 = 0.659 \quad (66\% \text{ probabilidad de colisión})
\end{equation}

Eficiencia del canal en función del número de nodos (asumiendo tráfico Poisson con carga ofrecida $\lambda$):

\begin{equation}
S = \frac{G \cdot e^{-G}}{1 + 2a}
\end{equation}
...
\begin{table}[h]
\centering
\caption{Eficiencia CSMA/CA vs Número de Nodos (p=0.1)}
\label{tab:csma-efficiency}
\begin{tabular}{|c|c|c|c|}
\hline
\textbf{Número de Nodos} & \textbf{Throughput (kbps)} & \textbf{Avg Delay (ms)} & \textbf{Collision Rate (\%)} \\
\hline
5 & 220 & 12 & 8 \\
10 & 195 & 28 & 18 \\
25 & 140 & 85 & 35 \\
...
\begin{table}[h]
\centering
\caption{Mapeo Capas ISO/IEC 30141 a Componentes de la Arquitectura}
\label{tab:iso30141-mapping}
\begin{tabular}{|p{4cm}|p{10cm}|}
\hline
\textbf{Entidad Funcional ISO/IEC 30141} & \textbf{Componente Implementado} \\
\hline
...
\begin{enumerate}
\item \textbf{Compromise de dispositivos}: Dispositivos resource-constrained son vulnerables a ataques de firmware (ej. Mirai botnet).
...
\item \textbf{Data exfiltration}: Acceso no autorizado a datos de telemetría sensibles.
\end{enumerate}
