...
\section{Configuraciones UCI Base del Gateway (BCM2711)}
...
\section{Configuraciones Router MT7628 (Routers Intermedios)}

Esta sección documenta las configuraciones UCI para routers basados en SoC MediaTek MT7628AN (MIPS 24KEc @ 580 MHz) utilizados como extensores de red mesh~\cite{morsemicroOpenWRT2024}. Estos routers implementan el target \texttt{ramips/mt76x8} de OpenWRT y funcionan exclusivamente como Layer-2/Layer-3 forwarding devices sin capacidades de edge computing.

\subsection{Especificaciones Hardware del Router MT7628}

\textbf{Modelos comerciales compatibles}:
\begin{itemize}
    \item GL.iNet GL-MT300N-V2 (128 MB RAM, 16 MB Flash, PoE opcional)
    \item HiLink HLK-7628N (128 MB RAM, 32 MB Flash, industrial)
    \item Widora NEO (256 MB RAM, 32 MB Flash, dev board)
\end{itemize}

\textbf{Características integradas}:
\begin{itemize}
    \item CPU: MIPS 24KEc single-core @ 580 MHz
    \item RAM: 128-256 MB DDR2
    \item Flash: 16-32 MB NOR (SPI)
    \item WiFi: 802.11n 2.4 GHz 2T2R integrado (kmod-mt7603)
    \item Ethernet: 5× Fast Ethernet 10/100 Mbps (switch integrado)
    \item PoE: 802.3af (12.95W) en modelos compatibles
\end{itemize}

\subsection{Network Configuration (/etc/config/network) - Router MT7628}
...
\subsection{Wireless Configuration (/etc/config/wireless) - Router MT7628}
...
\subsection{DHCP and DNS (/etc/config/dhcp) - Router MT7628}
...
\subsection{Firewall Simplificado (/etc/config/firewall) - Router MT7628}
...
\subsection{System Configuration (/etc/config/system) - Router MT7628}
...
\subsection{Optimizaciones de Performance para MT7628}
...
\begin{itemize}
    \item \textbf{Gateway BCM2711}: Configuraciones UCI de red, wireless HaLow, DHCP/DNS, firewall avanzado
    \item \textbf{Router MT7628}: Configuraciones UCI optimizadas para mesh forwarding con MT7603 WiFi
...
Todas las configuraciones están optimizadas para el hardware Raspberry Pi 4 (bcm27xx/bcm2711) y MediaTek MT7628 (ramips/mt76x8) con OpenWRT 23.05 basado en el repositorio oficial Morse Micro~\cite{morsemicroOpenWRT2024}, soportando los requisitos de resiliencia y seguridad del sistema de telemetría Smart Energy.
