...
\usetikzlibrary{calc,arrows.meta,positioning,shapes.geometric}% per right to e left to
\tikzset{
...
\include{03Arquitectura_NEW} % Arquitectura del Sistema: 4 Niveles Jerárquicos Campo→Fibra+HaLow→Gateway→Server (REORGANIZADO)
\include{04Implementacion_NEW} % Implementación Práctica del Sistema (NUEVO Cap 4)
\include{05Resultados_NEW} % Resultados Experimentales y Validación (NUEVO Cap 5)
%\include{04Gateway_NEW} % DEPRECADO - Comentado por itemize sin cerrar, contenido ya migrado a Cap 4/5
...
\include{15AnexoG_HipotesisDetalladas}
\end{appendix}
