...
\begin{itemize}
\item \textbf{Reducción de latencia >60\%:} La arquitectura propuesta logró latencia extremo-a-extremo (\textit{end-to-end}) estimada de 248 ms (calculada por suma de componentes individuales medidos, ver Cap 4 §4.13.1) vs 3247±118 ms en arquitectura centrada en la nube (\textit{cloud-centric}) línea base (\textit{baseline}), representando reducción estimada de 92.4\%. Adicionalmente, la latencia de procesamiento en el borde (\textit{edge}) fue medida experimentalmente alcanzando 8±2 ms promedio (P99=12 ms), cumpliendo requisitos IEC 62056 (<1s) para telemetría AMI con margen de 75\%.\footnote{Latencia E2E 248 ms no fue medida experimentalmente por limitaciones sincronización temporal (medidores sin NTP). Estimación basada en metodología latency budgeting IEC 62056-2021, validada en deployment piloto 30 medidores 90 días.}
