...
El gateway expone una API REST HTTP/TLS compatible con IEEE 2030.5. El siguiente ejemplo ilustra una consulta típica de telemetría:
...
\begin{enumerate}
    ...
    \item \textbf{RNF-GW-002}: Disponibilidad mayor al 99.5\% con failover automático Ethernet $\leftrightarrow$ LTE en <30 seg.
...
\begin{itemize}
    \item \textbf{Adquisición de datos}: Sensores de temperatura, humedad, consumo eléctrico, calidad del aire, etc.
    \item \textbf{Actuación}: Control de válvulas, relés, displays, alarmas.
    \item \textbf{Conectividad}: Thread (802.15.4) o HaLow 802.11ah en modo cliente (STA).
    \item \textbf{Protocolos de aplicación}: LwM2M (Lightweight M2M) sobre CoAP, MQTT-SN, IEEE 2030.5 Client.
\end{itemize}

\subsubsection{Características de Nodos IoT}

\begin{itemize}
    \item \textbf{Hardware}: Microcontroladores de bajo consumo (Arm Cortex-M4/M33): ESP32-C6, nRF52840, STM32U5, etc.
    \item \textbf{RAM/Flash}: 256 KB - 1 MB RAM, 1-4 MB Flash.
    \item \textbf{Modos de ahorro}: Sleep profundo, wake-on-radio, duty cycling < 1\%.
    \item \textbf{Autonomía}: 5-10 años con batería AA (2600 mAh) en aplicaciones de telemetría con reportes cada 15 min.
    \item \textbf{Stack de software}: RTOS ligero (FreeRTOS, Zephyr), stack Thread/Matter o driver HaLow STA.
\end{itemize}

\textbf{Ejemplo de implementación}: Nodo de telemetría de temperatura y humedad basado en ESP32 con ESP-IDF, cliente LwM2M (AVSystems Anjay), conectividad Thread vía nRF52840 RCP. La implementación de referencia se detalla en el Anexo E.
...
\begin{itemize}
    \item \textbf{Routing multi-hop}: Mesh 802.11s, EasyMesh (IEEE 1905.1), Thread Router.
    \item \textbf{Conectividad}: HaLow en modo AP + STA simultáneo (repetidor), Thread Router.
    \item \textbf{Funciones limitadas}: Forwarding de paquetes, discovery de vecinos, sin procesamiento de aplicación.
\end{itemize}

\subsubsection{Características de Routers IoT}

\begin{itemize}
    \item \textbf{Hardware}: SoC HaLow Morse Micro MM8108 + MPU Linux ligero (MediaTek MT7981B, MT7688).
    \item \textbf{RAM/Flash}: 128-256 MB RAM, 16-32 MB Flash.
    \item \textbf{Sistema Operativo}: OpenWRT mínimo, sin Docker ni bases de datos.
    \item \textbf{Alimentación}: PoE (802.3af/at), DC 12V, ocasionalmente baterías de respaldo.
    \item \textbf{Ejemplos comerciales}: GLi.net GL-MT3000 con módulo MM8108 (GLi.net GL-MT3000 + MM8108-EKH19 EVK)\cite{morse_micro_mm8108_2025}.
\end{itemize}

\subsubsection{Diferencias con Gateways}

Los routers IoT \textbf{no} implementan:
\begin{itemize}
    \item Border Router completo (sin traducción Thread $\leftrightarrow$ IP externa).
    \item Procesamiento edge (sin Docker, sin bases de datos).
    \item Conectividad WAN celular (LTE/5G).
    \item Interfaces IEEE 2030.5 Server o ThingsBoard Edge.
\end{itemize}

Su función es puramente de \textbf{extensión de cobertura} y \textbf{densificación de la red}, permitiendo que nodos IoT alejados del gateway alcancen conectividad mediante saltos intermedios.
...
\subsubsection{Características de Gateways IoT}

\begin{itemize}
    \item \textbf{Hardware}: Plataformas ARM Cortex-A con múltiples núcleos: Raspberry Pi 4 (BCM2711), Banana Pi, Orange Pi, NanoPi R5S.
    \item \textbf{RAM/Flash}: 4-8 GB RAM, 64-256 GB NVMe SSD.
    \item \textbf{Conectividad múltiple}: HaLow AP/Mesh, Thread Border Router, LTE/5G, Ethernet Gigabit, Wi-Fi 6.
    \item \textbf{Sistema Operativo}: OpenWRT 23.05 con Docker, UCI, kernel completo con módulos NVMe/USB/SPI.
    \item \textbf{Funciones avanzadas}:
    \begin{itemize}
        \item \textbf{Border Routing}: Thread $\leftrightarrow$ IPv6, HaLow $\leftrightarrow$ Ethernet/LTE.
        \item \textbf{Edge Computing}: ThingsBoard Edge, PostgreSQL, TimescaleDB, reglas locales, dashboards.
        \item \textbf{Protocolos Smart Energy}: IEEE 2030.5 Server (SEP 2.0), OpenADR, OCPP.
        \item \textbf{Message Brokers}: Apache Kafka, Mosquitto MQTT, RabbitMQ.
        \item \textbf{Orquestación}: Docker Compose, actualizaciones OTA de contenedores.
        \item \textbf{Almacenamiento local}: PostgreSQL + TimescaleDB para series temporales, buffer en caso de desconexión cloud.
    \end{itemize}
    \item \textbf{Alimentación}: PoE+ (802.3at/bt, 25-60W), DC 12V/5A.
\end{itemize}

\subsubsection{Justificación del Modelo de 3 Niveles}

La arquitectura jerárquica ofrece ventajas significativas frente a topologías planas:

\begin{enumerate}
    \item \textbf{Escalabilidad}: Un gateway puede gestionar 100-200 nodos directamente. Con routers intermedios, la red puede escalar a 1000+ nodos mediante saltos múltiples.
    \item \textbf{Eficiencia energética}: Los nodos no necesitan potencia de transmisión alta para alcanzar el gateway; pueden comunicarse con routers cercanos en saltos cortos (menor consumo).
    \item \textbf{Cobertura extendida}: HaLow ofrece >1 km en línea de vista. Con routers mesh, la cobertura puede alcanzar 3-5 km en entornos urbanos.
    \item \textbf{Resiliencia}: Si un router falla, el protocolo mesh reconfigura rutas automáticamente. El gateway permanece como punto de agregación estable.
    \item \textbf{Distribución de carga}: El gateway concentra procesamiento pesado (bases de datos, analytics, cloud sync); los routers solo routing; los nodos solo sensing.
    \item \textbf{Costo optimizado}: Desplegar pocos gateways costosos (RPi4 + NVMe + LTE) y múltiples routers económicos (MM8108 + MediaTek ~\$30 USD) es más rentable que múltiples gateways.
\end{enumerate}

\subsection{Flujo de Datos en Arquitectura de 3 Niveles}

El flujo típico de telemetría en la arquitectura propuesta sigue la siguiente secuencia:

\begin{enumerate}
    \item \textbf{Nodo IoT} (Nivel 1): El sensor adquiere temperatura (23.5°C) y la empaqueta en mensaje LwM2M/CoAP o IEEE 2030.5 XML.
    \item \textbf{Transmisión Thread/HaLow}: El nodo transmite el mensaje al router más cercano (RSSI óptimo) mediante protocolo Thread o HaLow STA.
    \item \textbf{Router IoT} (Nivel 2): Recibe el mensaje, verifica integridad, y lo reenvía al gateway mediante routing mesh (posiblemente con saltos intermedios adicionales).
    \item \textbf{Gateway IoT} (Nivel 3): Recibe el mensaje en interfaz Thread (OTBR) o HaLow (wlan-ah0).
    \item \textbf{Procesamiento Edge}: El gateway ejecuta:
    \begin{itemize}
        \item Conversión de protocolo (CoAP → MQTT, IEEE 2030.5 → JSON).
        \item Validación de datos (rango esperado, detección de anomalías).
        \item Almacenamiento en PostgreSQL/TimescaleDB.
        \item Publicación en topic Kafka para analytics locales.
        \item Envío a ThingsBoard Edge para visualización en dashboard local.
    \end{itemize}
    \item \textbf{Sincronización Cloud}: El gateway sincroniza datos con ThingsBoard Cloud vía LTE/Ethernet (cuando hay conectividad). Si no hay WAN, los datos quedan en buffer local hasta recuperar conectividad.
\end{enumerate}

En sentido inverso (comandos downlink desde cloud), el flujo es:

\begin{enumerate}
    \item \textbf{ThingsBoard Cloud} envía comando RPC al gateway (ej: apagar actuador).
    \item \textbf{Gateway} recibe comando, valida permisos (RBAC), y traduce a protocolo del nodo (LwM2M Write, IEEE 2030.5 DER Control).
    \item \textbf{Router(s)} reenvía(n) el comando al nodo destino mediante mesh.
    \item \textbf{Nodo} ejecuta acción (apagar relé) y responde con ACK.
    \item \textbf{Gateway} confirma ejecución a cloud.
\end{enumerate}

\section{Arquitectura de Software del Gateway}

El presente capítulo se enfoca en el \textbf{Nivel 3 (Gateway IoT)}, desarrollando en profundidad su arquitectura de software, hardware, configuración y despliegue. Los niveles 1 y 2 se referencian cuando sea necesario para contextualizar la integración.
...
El gateway integra múltiples interfaces de red mediante configuraciones UCI (Unified Configuration Interface) de OpenWRT:

\begin{itemize}
    \item \textbf{Thread 802.15.4}: OpenThread Border Router (OTBR) con nRF52840 RCP via USB (\texttt{/dev/ttyACM0}), forma red \texttt{SmartGrid-Thread} en canal 15
    \item \textbf{HaLow 802.11ah}: Tarjeta MM6108 via SPI, soporta 4 modos (AP Router, STA Client, Mesh 802.11s, EasyMesh 1905.1)
    \item \textbf{LTE Cat-M1/NB-IoT}: Quectel BG95-M3 via USB, gestionado por ModemManager, interfaz \texttt{wwan0} con failover vía mwan3
    \item \textbf{Ethernet Gigabit}: \texttt{eth0} como WAN primaria, enlace 1000BASE-T a ISP o red corporativa
\end{itemize}

Las configuraciones UCI completas para HaLow en sus cuatro modos de operación (AP Router con NAT, STA Client, Mesh 802.11s, EasyMesh 1905.1 Controller/Agent) se presentan en el Anexo D. Los comandos de inicialización de OTBR y LTE modem se documentan en el Anexo B con los docker-compose correspondientes.
...
# Esperado: "leader" o "router" (red Thread activa)

# HaLow 802.11ah
iw dev wlan2 info
# Esperado: Interface wlan2, type AP, channel 7 (917 MHz)

# LTE modem
mmcli -m 0 --simple-status
# Esperado: state: connected, signal-quality: 75%

# Ethernet WAN
cat /sys/class/net/eth0/operstate
# Esperado: up (enlace 1000BASE-T activo)
\end{verbatim}
...
\subsection{Despliegue de Servicios en Docker}

La plataforma del gateway se despliega mediante contenedores Docker orquestados con docker-compose. Los servicios principales son:

\begin{itemize}
    \item \textbf{OpenThread Border Router (OTBR)}: Enrutamiento Thread/IPv6 (\texttt{/dev/ttyUSB0} → nRF52840 RCP)
    \item \textbf{ThingsBoard Edge}: Plataforma IoT local con sincronización Cloud
    \item \textbf{PostgreSQL + TimescaleDB}: Base de datos de series temporales
    \item \textbf{IEEE 2030.5 Server (SEP 2.0)}: API REST estándar para utilidades
    \item \textbf{Bridge Thread$\leftrightarrow$TB}: Traductor MQTT entre redes Thread y ThingsBoard
    \item \textbf{Apache Kafka}: Bus de mensajes para integración empresarial
\end{itemize}

Los archivos \texttt{docker-compose.yml} completos para OTBR, ThingsBoard Edge, PostgreSQL y servicios auxiliares se presentan en el Anexo B. Los scripts Python de implementación para el IEEE 2030.5 Server, el Bridge Thread$\leftrightarrow$ThingsBoard, y los productores/consumidores Kafka se documentan en el Anexo C.

El despliegue típico sigue esta secuencia:

\begin{verbatim}
cd /mnt/docker/otbr && docker-compose up -d
cd /mnt/docker/tb-edge && docker-compose up -d
cd /mnt/docker/sep20-server && docker-compose up -d
cd /mnt/docker/thread-bridge && docker-compose up -d
docker ps  # Verificar todos los servicios activos
\end{verbatim}
...
TimescaleDB es una extensión de PostgreSQL optimizada para series temporales, que ofrece:

\begin{itemize}
    \item \textbf{Compresión}: 10-20× reducción en espacio disco mediante algoritmos adaptativos
    \item \textbf{Queries rápidas}: Particionamiento automático por tiempo (chunks de 7 días por defecto)
    \item \textbf{Agregaciones}: Función \texttt{time\_bucket} para promedios 15-min/1-hora con vistas materializadas
    \item \textbf{Retención automática}: Elimina datos antiguos sin intervención manual
\end{itemize}

El esquema completo de TimescaleDB, incluyendo la definición de hypertables, políticas de compresión, continuous aggregates (vistas materializadas para agregaciones de 15-min, 1-hora y 1-día), políticas de retención (90 días), y cinco consultas SQL de ejemplo (telemetría reciente, consumo diario, alertas de voltaje, promedios semanales y agregados de facturación mensual), se presenta en el Anexo D. El docker-compose de TimescaleDB se documenta en el Anexo B.

Ejemplo de consulta para consumo promedio por hora:
...
\begin{itemize}
    \item \textbf{Gestión de dispositivos}: Bootstrap, configuración remota, firmware OTA
...
Los objetos LwM2M utilizados incluyen:
\begin{itemize}
    \item \textbf{Objeto 0 (Security)}: Server URI, PSK, Bootstrap
    \item \textbf{Objeto 1 (Server)}: Lifetime, Binding, Registration
    \item \textbf{Objeto 3 (Device)}: Manufacturer, Model, Serial Number, Battery, Memory, Reboot
    \item \textbf{Objeto 4 (Connectivity)}: Network Bearer, IP Address, Signal Strength
    \item \textbf{Objeto 5 (Firmware Update)}: Package URI, Update State, Execute Update
    \item \textbf{Objeto 6 (Location)}: Latitude, Longitude, Timestamp
    \item \textbf{Objeto 3303 (Temperature)}: Sensor Value, Units, Min/Max, Reset
    \item \textbf{Objeto 3304 (Humidity)}: Sensor Value, Units, Min/Max
    \item \textbf{Objeto 3312 (On/Off)}: Digital Output State (boolean)
\end{itemize}

La implementación completa de referencia de un nodo IoT ESP32-C6 con cliente LwM2M AVSystems Anjay, incluyendo todos los archivos de código fuente (\texttt{main.c}, \texttt{lwm2m\_client.c}, \texttt{temp\_object.c}, \texttt{humidity\_object.c}, \texttt{device\_object.c}, \texttt{connectivity\_object.c}, \texttt{location\_object.c}, \texttt{onoff\_object.c}, \texttt{thread\_prov.c}), el \texttt{CMakeLists.txt}, instrucciones de compilación con ESP-IDF, y comandos de comisionamiento Thread/LwM2M, se documenta en el Anexo E.

\subsubsection{Ventajas LwM2M para Smart Metering}
\begin{itemize}
