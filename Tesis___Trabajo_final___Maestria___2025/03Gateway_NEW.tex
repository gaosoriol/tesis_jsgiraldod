...
La implementación de referencia para nodos IoT en esta arquitectura se basa en el repositorio \texttt{github.com/jsebgiraldo/OpenThread-LwM2M}, que integra la pila OpenThread 1.3~\cite{ThreadGroup2023} con el cliente LwM2M de Eclipse Wakaama~\cite{OMA_LwM2M2020}, proporcionando una solución completa de conectividad mesh 802.15.4 con gestión de dispositivos estandarizada.
...
\begin{itemize}
    \item \textbf{Thread 1.3}\footnote{Implementación del piloto con Thread 1.3.0 (OpenThread stack). Thread 1.4.0 recomendado para producción (véase §4.4).}~\cite{ThreadGroup2023}: IPv6 sobre 802.15.4 con routing mesh, puesta en servicio (\textit{comisionamiento}) seguro (PAKE), multicast confiable
...
Los routers del Nivel 2 constituyen los elementos de extensión de cobertura en la arquitectura, implementando tecnología IEEE 802.11ah (Wi-Fi HaLow) para proveer conectividad de largo alcance (>1 km) con bajo consumo energético entre los nodos IoT de campo y la pasarela de borde~\cite{IEEE80211ah2016,ahmedMACProtocolsIEEE2022}. A diferencia de Wi-Fi convencional (2.4/5 GHz), HaLow opera en bandas sub-GHz (902-928 MHz en América) con características superiores de propagación y penetración de obstáculos, especialmente idóneo para aplicaciones IoT de gran escala~\cite{ahmedMACProtocolsIEEE2022}.
...
Validación experimental: Alcance 1.8 km LOS @ MCS0 (RSSI -92 dBm), throughput 4.2 Mbps 1-hop / 1.1 Mbps 4-hop, latencia 18 ms / 85 ms, 180 dispositivos simultáneos por router. Estos valores son consistentes con estudios recientes de desempeño IEEE 802.11ah que demuestran capacidades de largo alcance (>1 km) y throughput adecuado para aplicaciones IoT~\cite{amrilPerformanceEvaluationWiFi2025}.
...
Configuración tradicional Access Point - Station: Un router HaLow AP central (Alfa Tube-AHM) sirve hasta 8192 dispositivos según especificación IEEE 802.11ah (Association ID de 13 bits)~\cite{IEEE80211ah2016,ahmedMACProtocolsIEEE2022}, aunque implementaciones comerciales limitan a 250-500 STAs simultáneos por restricciones CPU/memoria. 
