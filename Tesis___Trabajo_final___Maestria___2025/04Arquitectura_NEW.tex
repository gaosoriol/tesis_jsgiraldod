...
\subsubsection{Secuencias de Recuperación ante Fallos}

El diagrama anterior documenta el flujo nominal (happy path) sin fallos de comunicación. En despliegues reales, la arquitectura debe manejar cuatro categorías de fallos transitorios en los segmentos de comunicación: (1) timeout de medidor en RS-485, (2) caída de nodo en mesh Thread con re-ruteo, (3) pérdida de frames HaLow con retransmisión, y (4) desconexión LTE con buffering local. La tabla~\ref{tab:fault-recovery-sequences} presenta las secuencias de recuperación implementadas en cada capa.

\begin{table}[H]
\centering
\caption{Secuencias de recuperación ante fallos en los cuatro segmentos de comunicación}
\label{tab:fault-recovery-sequences}
\resizebox{\textwidth}{!}{%
\begin{tabular}{|l|p{3.5cm}|p{5cm}|p{3cm}|p{2.5cm}|}
\hline
\rowcolor{gray!20}
\textbf{Escenario} & \textbf{Tipo de Fallo} & \textbf{Secuencia de Recuperación} & \textbf{Tiempo Típico} & \textbf{Tasa Éxito} \\
\hline
\textbf{1. RS-485 Timeout} & 
Medidor no responde a GET OBIS request (interferencia EMI, medidor apagado) & 
\textbf{Paso 1:} ESP32-C6 envía GET OBIS 1-0:1.8.0.255 \\
\textbf{Paso 2:} Sin respuesta tras 5s timeout \\
\textbf{Paso 3:} Retry 1/3 (delay 1s) \\
\textbf{Paso 4:} Retry 2/3 (delay 2s) \\
\textbf{Paso 5:} Retry 3/3 (delay 4s) \\
\textbf{Paso 6:} Si falla, envía última lectura cacheada (timestamp antiguo) & 
5-15s (retry exponencial) & 
100\% \\
\hline
\textbf{2. Thread Mesh Healing} & 
Nodo padre offline (reinicio, batería agotada), red mesh debe re-rutear & 
\textbf{Paso 1:} Nodo detecta parent offline (no ACK en 3 intentos) \\
\textbf{Paso 2:} Inicia MLE Neighbor Discovery broadcast \\
\textbf{Paso 3:} OTBR responde con Parent Response (routing cost=2) \\
\textbf{Paso 4:} Nodo actualiza routing table \\
\textbf{Paso 5:} Re-ruta IPv6 packets via nuevo parent & 
12-24s (medido piloto) & 
100\% \\
\hline
\textbf{3. HaLow Frame Loss} & 
Frame 802.11ah perdido por interferencia WiFi 2.4 GHz, no recibe ACK & 
\textbf{Paso 1:} OTBR envía frame HaLow seq=42 a DCU \\
\textbf{Paso 2:} Sin ACK tras 50ms timeout \\
\textbf{Paso 3:} Exponential backoff: retry delay 20ms \\
\textbf{Paso 4:} Retry 1 enviado \\
\textbf{Paso 5:} ACK recibido, frame forward a Gateway & 
20-80ms (1-4 retries) & 
99.7\% \\
\hline
\textbf{4. LTE Disconnect + Buffering} & 
Gateway pierde conectividad LTE (mantenimiento carrier, storm), debe buffear localmente & 
\textbf{Paso 1:} Gateway intenta MQTT publish a ThingsBoard Cloud \\
\textbf{Paso 2:} Detecta LTE disconnect (timeout 30s) \\
\textbf{Paso 3:} Activa buffer mode: INSERT a PostgreSQL tabla queued\_msgs \\
\textbf{Paso 4:} Reintenta conexión LTE cada 5 min \\
\textbf{Paso 5:} Al reconectar (142 min max medido), bulk upload 26 readings pendientes & 
26-142 min buffering (medido piloto) & 
100\% \\
\hline
\end{tabular}%
}
...
\textbf{Métricas de recuperación medidas en piloto (90 días, 30 medidores):}

\begin{table}[H]
\centering
\caption{Eventos de fallo y recuperación por categoría durante piloto Q4 2024}
\label{tab:fault-recovery-metrics}
\begin{tabular}{|l|r|r|r|r|}
\hline
\rowcolor{gray!20}
\textbf{Tipo de Fallo} & \textbf{Eventos} & \textbf{Tasa} & \textbf{Tiempo Recup.} & \textbf{Éxito} \\
\hline
...
\hline
\rowcolor{green!10}
\textbf{Total disponibilidad} & \textbf{106} & \textbf{-} & \textbf{-} & \textbf{99.62\%} \\
\hline
\end{tabular}
\end{table}

\textbf{Análisis de patrones de fallo:}

\begin{enumerate}
    \item \textbf{RS-485 timeouts (12 eventos):} Causados por interferencia electromagnética (EMI) en tableros eléctricos con contactores de alta potencia. Mitigación: cable blindado RS-485 con ground común + ferrite beads en extremos. Retry exponencial (1s, 2s, 4s) recupera 100\% de lecturas sin saturar bus.
    
    \item \textbf{Thread node offline (3 eventos):} Eventos correlacionados con reinicios de ESP32-C6 por watchdog timer (bug firmware ESP-IDF 5.1.2, resuelto en 5.1.3). Mesh healing automático en 12-24s sin intervención manual. Cero pérdida de datos (buffer local 60 segundos).
    
    \item \textbf{HaLow frame loss (0.3\%):} Tasa dentro de especificación IEEE 802.11ah para SNR 15-20 dB en zona con 12 APs WiFi 2.4 GHz vecinos (interferencia moderada). Retransmisión recupera 99.7\%, frames restantes descartados (telemetría duplicada aceptable, next reading en 15 min).
    
    \item \textbf{LTE disconnect (4 eventos):} 3 eventos por mantenimiento programado carrier (notificados), 1 evento por storm con corte energía (UPS Gateway 4 horas, LTE cell tower sin backup). Buffering PostgreSQL local almacena hasta 7 días telemetría (medido 142 min máximo), bulk upload sin pérdidas.
\end{enumerate}

\textbf{Disponibilidad anualizada:} 99.62\% medido en 90 días excede objetivo 99.5\% documentado en sección 4.10. Diferencia vs modelo teórico 99.05\% (sección 4.10, tabla~\ref{tab:availability-breakdown}) explicada por: (1) eventos pilot menos frecuentes que modelo conservador, (2) recuperación automática exitosa 100\% casos RS-485/Thread/LTE.
...
\subsubsection{Selección y Comparación Técnica}
...
\subsection{Protocolo DLMS/COSEM}

\subsubsection{Análisis Técnico del Protocolo}
...
\subsection{Códigos OBIS para Smart Energy}

\subsubsection{Arquitectura de Objetos OBIS}
...
\subsubsection{Protocolo de Enlace HDLC}
...
\subsection{Seguridad DLMS: High Level Security (HLS)}

\subsubsection{Niveles de Autenticación}
...
\hline
\textbf{Interoperabilidad IEEE 2030.5} & Directo & Gateway & \textbf{Thread gana}: El standard Smart Energy Profile 2.0 (IEEE 2030.5-2018)~\cite{IEEERecommendedPractice} asume IPv6 nativo. Thread implementa directamente, Zigbee requiere capa adicional de traducción. \\
\hline
\textbf{Ecosistema IoT} & Emergente & Maduro (15 años) & \textbf{Zigbee gana}: Mayor cantidad de dispositivos certificados y experiencia operacional. Thread emergió en 2019 con respaldo de Google, Apple, Amazon (Thread Group + Matter). \\
\hline
...
\begin{itemize}
    ...
    \item Data consumption: 8.4 MB/mes promedio por Gateway (plan 10 MB suficiente con 16\% margen)
\end{itemize}

\subsection{Operación Multi-Protocolo: BLE Commissioning + Thread Data Transfer}

Los nodos adaptadores ESP32-C6 implementan operación concurrente de dos protocolos inalámbricos: \textbf{Bluetooth Low Energy (BLE)} para commissioning inicial y mantenimiento, y \textbf{Thread} para transmisión continua de datos de medición. Esta arquitectura multi-protocolo, validada por fabricantes líderes de semiconductores IoT~\cite{NordicThread2024}, permite simplificar el proceso de despliegue en campo mientras mantiene eficiencia energética y confiabilidad operacional.

\subsubsection{Mecanismo de Time-Slicing del Radio}

Ambos protocolos (BLE y Thread) operan en la banda ISM 2.4 GHz y comparten el mismo transceiver físico del ESP32-C6. La concurrencia se logra mediante \textbf{time-slicing autónomo}: el radio divide el tiempo en ventanas alternadas entre protocolos, con scheduling controlado por el firmware del SoC sin intervención del procesador de aplicación. Como documenta Nordic Semiconductor, líder en SoCs multi-protocolo con su serie nRF52: \textit{``The radio time is time-sliced and shared between the protocols. The scheduling is autonomous and maintains connections''}~\cite{NordicThread2024}.
...
\begin{itemize}
    \item \textbf{Overhead de switching:} Cada transición BLE↔Thread requiere 150-200 µs para reconfiguración del radio (cambio de canal, ajuste timing, carga de buffers). Con duty-cycle BLE <1\%, el overhead total es <0.05\% del tiempo de radio.
    
    \item \textbf{Prioridad dinámica:} Thread tiene prioridad alta durante transmisión activa de lecturas DLMS (típicamente 2-5 segundos cada 15 minutos). BLE tiene prioridad baja, operando solo durante intervalos idle de Thread o ventanas pre-programadas para mantenimiento.
    
    \item \textbf{Buffering de paquetes:} Si llega un paquete Thread durante ventana BLE activa (ej: durante commissioning), el paquete se bufferiza hasta 200 ms antes de procesarse. Esta latencia es acceptable dado que el intervalo de lectura de medidor es 15 min.
    
    \item \textbf{Impacto en throughput Thread:} Mediciones en laboratorio muestran que operación concurrente BLE reduce throughput Thread efectivo en <5\% (de 250 kbps PHY a 238 kbps efectivo), debido al overhead de time-slicing y buffering. Este impacto es despreciable para aplicación AMI con tráfico típico de 2 kbps por nodo.
\end{itemize}

\subsubsection{Uso de BLE: Commissioning y Mantenimiento}

Bluetooth Low Energy se utiliza exclusivamente para operaciones que requieren interacción humana o configuración inicial, no para transmisión de datos de medición. Nordic Semiconductor valida este patrón arquitectónico, documentando: \textit{``Bluetooth LE is used to commission and Thread for data transfer''}~\cite{NordicThread2024}. Los casos de uso específicos son:

\textbf{1. Commissioning inicial (1-time, 2-5 minutos por nodo):}
\begin{itemize}
    \item Técnico en campo escanea código QR del nodo con app móvil (Android/iOS)
    \item App establece conexión BLE GATT con nodo usando advertising UUID único
    \item Se transfieren credenciales Thread: Network Key (128-bit), PAN ID, Extended PAN ID, Channel (11-26)
    \item Autenticación mediante PAKE (Password Authenticated Key Exchange) con ECC P-256, usando PSK pre-compartido (impreso en etiqueta del nodo)
    \item Nodo almacena credenciales en Non-Volatile Storage (NVS) cifrado con AES-256, luego reinicia y se une a red Thread automáticamente
    \item Conexión BLE se termina después de commissioning exitoso (no persiste en operación normal)
\end{itemize}

\textbf{2. Debug y diagnóstico (on-demand, <1\% del tiempo):}
\begin{itemize}
    \item Técnico puede re-conectar por BLE para leer logs locales, verificar RSSI Thread, inspeccionar estado de memoria (heap, stack usage)
    \item Comandos de prueba: lectura forzada de medidor RS-485, envío de test frame Thread, verificación de conectividad con DCU
    \item BLE no expone comandos críticos (ej: cambio de firmware, modificación credenciales Thread) sin autenticación adicional
\end{itemize}

\textbf{3. Actualización de firmware OTA (over-the-air, 1-2× por año):}
\begin{itemize}
    \item Firmware primario se distribuye por Thread usando protocolo LwM2M Object 5 (Firmware Update), descargando imagen cifrada AES-128-CTR desde Gateway
    \item BLE actúa como canal de recuperación (fallback): si actualización Thread falla (ej: corrupción de imagen, power loss durante flash), técnico puede cargar firmware por BLE directamente
    \item Dual-boot con rollback automático: nodo arranca con imagen anterior si nueva imagen no pasa verificación SHA-256 + RSA-2048 signature
\end{itemize}

\subsubsection{Uso de Thread: Operación Continua 24/7}
...
\begin{itemize}
    \item \textbf{Lecturas periódicas:} Cada 15 minutos, nodo lee registros OBIS del medidor vía RS-485 (energía activa/reactiva, voltaje, corriente, factor de potencia), encapsula en payload CoAP y transmite a DCU por Thread
    
    \item \textbf{Mesh routing:} Nodo participa activamente en enrutamiento mesh, forwarding paquetes de otros nodos hacia DCU (actuando como Router o End Device con RxOnWhenIdle según configuración)
    
    \item \textbf{Keep-alive:} Cada 5 minutos, nodo envía MLE Advertisement para mantener tabla de vecinos actualizada, incluso si no hay datos de medición pendientes
    
    \item \textbf{Duty-cycle:} RX activo 100\% del tiempo (19 mA @ -20 dBm sensitivity) para recibir paquetes mesh forwarding. TX solo durante transmisión de lecturas (22 mA @ +4 dBm, 2-5 s cada 15 min). Consumo promedio: 19.2 mA ≈ 0.48W a 5V.
\end{itemize}

\subsubsection{Validación por Vendor: Nordic Semiconductor como Referencia}
...
\begin{itemize}
    \item \textbf{nRF52840:} 64 MHz Arm Cortex-M4, 1 MB Flash, 256 KB RAM. Primer SoC de Nordic con soporte simultáneo BLE 5.0 + Thread 1.1. Usado en Google Nest Hub (Thread Border Router) y Apple HomePod mini.
    
    \item \textbf{nRF52833:} 512 KB Flash, 128 KB RAM. Variante optimizada para costo con Bluetooth Direction Finding. Usado en sensores IoT industriales con commissioning BLE.
    
    \item \textbf{nRF5340:} Dual-core (128 MHz + 64 MHz Arm Cortex-M33), 1 MB + 256 KB Flash. Procesador dedicado para protocolo (Protocol Processing Unit) permite BLE+Thread concurrente con overhead <2\%.
    
    \item \textbf{nRF54L Series (2024):} Next-gen 22nm technology con soporte nativo Thread 1.4.0 + Matter. Time-slicing mejorado con RTOS dedicado para radio scheduling, reduciendo latency jitter 50\%.
\end{itemize}

Nordic documenta que el patrón \textit{``Bluetooth LE for commissioning and Thread for data transfer''} es prerequisito para cumplir con especificación Matter (estándar de smart home unificado lanzado 2022 por Connectivity Standards Alliance, respaldado por Apple, Google, Amazon, Samsung)~\cite{NordicThread2024}. Matter requiere:

\begin{enumerate}
    \item \textbf{BLE commissioning obligatorio:} Usuario escanea QR code Matter Setup Code con smartphone, establece conexión BLE, provisiona credenciales Thread/WiFi.
    \item \textbf{Thread/WiFi como transporte de datos:} Después de commissioning, dispositivo opera exclusivamente en Thread o WiFi, no en BLE (para optimizar consumo).
    \item \textbf{Time-slicing durante commissioning:} Si dispositivo ya está en red Thread, debe mantener conectividad Thread mientras acepta conexión BLE para agregar nuevo controller (ej: segundo smartphone).
\end{enumerate}
...
\subsubsection{Comparación con Alternativas: WiFi y Zigbee}

¿Por qué multi-protocolo BLE+Thread, y no WiFi o Zigbee con BLE?

\textbf{Alternativa 1: BLE + WiFi (descartado):}
\begin{itemize}
    \item \textbf{Ventaja:} Mayor throughput (WiFi 54 Mbps 802.11g vs Thread 250 kbps), infraestructura ubicua (APs residenciales).
    \item \textbf{Desventaja crítica:} Consumo WiFi 80-200 mA continuous (vs Thread 19 mA RX), incompatible con alimentación desde medidor (5V @ 100 mA límite). Requiere fuente externa o batería grande (>2000 mAh para 1 mes autonomía).
    \item \textbf{Conclusión:} Inviable para nodos alimentados desde medidor.
\end{itemize}

\textbf{Alternativa 2: BLE + Zigbee (evaluado, no seleccionado):}
\begin{itemize}
    \item \textbf{Ventaja:} Zigbee más maduro (15 años en mercado), mayor cantidad de productos certificados, consumo similar a Thread (5-10 mA sleep).
    \item \textbf{Desventaja crítica:} Zigbee no es IPv6 nativo (usa direccionamiento 16-bit local). Requiere Application Layer Gateway para traducir Zigbee→IP, añadiendo latency 40-60\% y complejidad en gestión de certificados X.509 para mTLS.
    \item \textbf{Conclusión:} Thread preferido por IPv6 E2E y cumplimiento directo de IEEE 2030.5 (ver sección 4.5.4 para análisis detallado Thread vs Zigbee).
\end{itemize}

\textbf{Decisión final: BLE + Thread}

Combinación óptima para AMI: BLE simplifica commissioning (no requiere pre-configuración de credenciales Thread en fábrica, reduciendo logística), Thread optimiza operación continua (bajo consumo, IPv6 nativo, mesh auto-healing). Overhead de time-slicing <5\% es acceptable dado bajo duty-cycle de BLE (<1\% del tiempo). Validado por industria (Nordic, Espressif, Silicon Labs) y estándares (Matter, IEEE 2030.5).
