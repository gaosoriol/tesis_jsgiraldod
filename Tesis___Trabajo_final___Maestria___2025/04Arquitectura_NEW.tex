...
Esta aclaración resuelve aparentes inconsistencias en cifras de sobrecarga mencionadas en diferentes contextos: cuando se habla de "sobrecarga CoAP 4-10 bytes" se refiere exclusivamente al encabezado CoAP, mientras que "22-28 bytes" incluye toda la pila de protocolos (\textit{stack}) completa (CoAP + UDP + IPv6 comprimido). La reducción del "78\%" o "72\%" documentada considera además el procesamiento en el borde (\textit{edge}) que filtra datos no críticos y agrega temporalmente, reduciendo el tráfico WAN final de 24.6 GB/día (línea base) a 6.9 GB/día (propuesta).
...
\hline
\textbf{Interoperabilidad IEEE 2030.5} & ✅ Directo & ⚠️ Gateway & \textbf{Thread gana}: El standard Smart Energy Profile 2.0 (IEEE 2030.5-2023)~\cite{IEEERecommendedPractice} asume IPv6 nativo. Thread implementa directamente, Zigbee requiere capa adicional de traducción. \\
\hline
...
\textbf{Decisión final: Thread 1.3.0}
...
\begin{table}[h]
\centering
\caption{Matriz de Vectores de Ataque, Impacto y Mitigaciones Implementadas}
\label{tab:security-threats}
\resizebox{\textwidth}{!}{%
\begin{tabular}{|p{3.5cm}|p{2cm}|p{2cm}|p{6cm}|p{3.5cm}|}
\hline
\rowcolor{gray!20}
\textbf{Vector de Ataque} & \textbf{Impacto} & \textbf{Probabilidad} & \textbf{Mitigación Implementada} & \textbf{Riesgo Residual} \\
\hline
\multicolumn{5}{|c|}{\cellcolor{blue!20}\textbf{CAPA 1: NODOS DE CAMPO (THREAD MESH)}} \\
\hline
\textbf{A1: Nodo comprometido} & Crítico & Baja & Thread Commissioning con PAKE (ECC P-256) + rotación de Network Key cada 90 días + Secure Boot habilitado en ESP32-C6 (firma RSA-2048 del firmware) & Medio: Requiere acceso físico al nodo para bypass de Secure Boot \\
\hline
\textbf{A2: Replay de mensajes Thread} & Alto & Media & Cada mensaje CoAP incluye timestamp Unix + Message ID único. Gateway descarta mensajes con delay >30s o MID duplicado en ventana de 100 mensajes. & Bajo: Ventana de 30s podría permitir replay si captura ocurre <30s \\
\hline
\textbf{A3: Physical tampering de nodo} & Alto & Media & Detección de apertura con reed switch + alarma inmediata al gateway + log inmutable en TimescaleDB con hash SHA-256. Caja con tornillos de seguridad Torx T10. & Medio: Alarma reactiva, no previene acceso físico \\
\hline
\multicolumn{5}{|c|}{\cellcolor{green!20}\textbf{CAPA 2: GATEWAY (OTBR + HALOW AP)}} \\
\hline
\textbf{A4: OTBR comprometido} & Crítico & Baja & SSH con autenticación dual: (1) Key-based (RSA-4096) + (2) Google Authenticator OTP. Root login deshabilitado. Firewall nftables drop all INPUT excepto 22/tcp desde IP whitelisted. & Bajo: Requiere compromiso de private key + OTP seed \\
\hline
\textbf{A5: Man-in-the-Middle en HaLow} & Alto & Media & HaLow con WPA3-SAE (Simultaneous Authentication of Equals) resiste diccionario offline~\cite{ieee80211ah2020}. Modo RAW permite detección de jamming (SNR <10 dB gatilla alarma). Channel hopping automático (2/4/8 MHz) ante interferencia. & Medio: Jamming sofisticado multi-banda podría causar DoS temporal \\
\hline
\textbf{A6: Exfiltración de datos PostgreSQL} & Crítico & Baja & TimescaleDB con: (1) Encriptación at-rest (LUKS dm-crypt AES-256-XTS), (2) mTLS para conexiones cliente-servidor (certificados X.509 renovados cada 90 días), (3) Row-Level Security (RLS) PostgreSQL limitando queries por rol. & Bajo: Requiere compromiso de certificado + bypass RLS \\
\hline
\multicolumn{5}{|c|}{\cellcolor{orange!20}\textbf{CAPA 3: BACKHAUL (LTE CAT-M1)}} \\
\hline
\textbf{A7: Interceptación tráfico MQTT} & Alto & Media & MQTT con TLS 1.3 (ChaCha20-Poly1305 cipher) + certificate pinning (fingerprint SHA-256 del broker hardcoded en cliente). Fallback a TLS 1.2 deshabilitado. & Bajo: Requiere compromiso de CA raíz para certificate substitution \\
\hline
\textbf{A8: Credential theft MQTT} & Alto & Media & Credenciales almacenadas en TPM 2.0 (Trusted Platform Module) del gateway. MQTT username/password rotados cada 30 días automáticamente vía script. Rate limiting: 5 intentos fallidos → bloqueo IP 1 hora. & Bajo: TPM resiste ataques físicos hasta nivel de laboratorio \\
\hline
\end{tabular}%
}
\end{table}

\subsubsection{Assessment NIST Cybersecurity Framework}

La arquitectura implementa controles alineados con las cinco funciones del NIST CSF: Identify, Protect, Detect, Respond, Recover. La evaluación por Tiers (niveles de madurez) posiciona la implementación actual en \textbf{Tier 3 (Repeatable)}, con roadmap hacia Tier 4 (Adaptive).

\textbf{Tier 3 - Repeatable (Estado Actual):}

\begin{itemize}
    \item \textbf{Identify:} Asset inventory automatizado (todos los nodos Thread registrados en base de datos con MAC, certificado X.509, ubicación física). Risk assessment documentado en esta sección.
    
    \item \textbf{Protect:} Controles técnicos implementados (mTLS, WPA3-SAE, TPM 2.0, AppArmor). Políticas de acceso basadas en roles (RBAC) con segregación Operator/Administrator.
    
    \item \textbf{Detect:} Monitoring continuo con Prometheus + Grafana. Alertas configuradas para 15 eventos críticos (nodo offline >5 min, CPU >90\%, failed SSH login, jamming detectado, certificate expiry <7 días).
    
    \item \textbf{Respond (Responder):} Manuales de procedimiento (\textit{Playbooks}) documentados para 8 incidentes comunes (nodo comprometido → revocación de certificado + re-comisionamiento; OTBR comprometido → aislamiento de red + restauración desde respaldo). Tiempo de respuesta objetivo <4 horas.
    
    \item \textbf{Recover (Recuperar):} Respaldos (\textit{Backups}) automáticos diarios de configuración UCI + base de datos TimescaleDB. Punto de Recuperación Objetivo (\textit{Recovery Point Objective} - RPO) = 24h, Tiempo de Recuperación Objetivo (\textit{Recovery Time Objective} - RTO) = 2h mediante restauración desde instantánea (\textit{snapshot}).
\end{itemize}
