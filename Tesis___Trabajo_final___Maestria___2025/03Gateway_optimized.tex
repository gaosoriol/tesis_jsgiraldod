...
\textbf{Protocolos}: Thread $\leftrightarrow$ MQTT, CoAP $\leftrightarrow$ HTTP, HaLow, LTE
...
IEEE 2030.5 Bridge & Custom & Protocol translator CoAP $\leftrightarrow$ MQTT (Python) \\ \hline
\end{tabular}
\end{table}
...
MQTT es el protocolo principal para comunicación gateway $\leftrightarrow$ cloud. Características:
...
CoAP es un protocolo REST sobre UDP optimizado para dispositivos con recursos limitados (RFC 7252). Usado en la comunicación medidores Thread $\leftrightarrow$ gateway.
