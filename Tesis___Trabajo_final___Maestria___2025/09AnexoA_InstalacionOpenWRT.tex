...
\begin{itemize}
    \item \textbf{Versión OpenWRT}: 23.05 (custom build desde Morse Micro fork)~\cite{morsemicroOpenWRT2024}
    \item \textbf{Repositorio fuente}: \texttt{https://github.com/MorseMicro/openwrt}
    \item \textbf{Base upstream}: Backports mac80211 6.1.110-1 (inalámbrico) + OpenWRT 23.05 (core)
    \item \textbf{Target}: \texttt{bcm27xx/bcm2711} (Raspberry Pi 4 specific, 64-bit ARMv8)
    \item \textbf{Subtarget device}: \texttt{rpi-4-mmeval} (Morse Micro evaluation configuration)
    \item \textbf{Kernel}: Linux 5.15 LTS (con patches BCM2711 y driver Morse Micro)
    \item \textbf{Arquitectura binarios}: \texttt{aarch64\_cortex-a72} (ARM64v8, optimizado para Cortex-A72)
    \item \textbf{Libc}: musl 1.2.4 (lightweight C library)
    \item \textbf{Bootloader}: Raspberry Pi firmware (start4.elf en FAT32 boot partition)
    \item \textbf{Device tree overlays}: \texttt{mm610x-spi}, \texttt{mm810x-spi}, \texttt{mm\_wlan}, \texttt{morse-ps}
\end{itemize}

\subsection{Build desde Repositorio Morse Micro (Opcional Avanzado)}
...
\textbf{NOTA}: Para despliegues estándares se recomienda usar las imágenes pre-compiladas oficiales de Morse Micro disponibles en su portal de desarrolladores.

\subsubsection{Requisitos del Sistema de Compilación}

\textbf{Hardware mínimo recomendado}:
\begin{itemize}
    \item CPU: x86\_64 con 4+ cores (compilación multi-thread)
    \item RAM: 16 GB mínimo (se requieren >8 GB durante linking)
    \item Almacenamiento: 60 GB libres (sources + build artifacts)
    \item Sistema operativo: Ubuntu 20.04 LTS o superior
\end{itemize}

\textbf{Instalación de dependencias en Ubuntu}:
\begin{verbatim}
...
\subsubsection{Clonación y Configuración del Repositorio}
...
\subsubsection{Paquetes Específicos HaLow Incluidos en el Build}

El build de Morse Micro incluye automáticamente los siguientes paquetes propietarios no disponibles en OpenWRT upstream:

\begin{itemize}
    \item \texttt{kmod-morse}: Módulo kernel driver para chipsets MM6108/MM8108 (802.11ah PHY/MAC)
    \item \texttt{morse-fw-6108}: Firmware binario para MM6108 (versión 2x2 MIMO)
    \item \texttt{morse-fw-8108}: Firmware binario para MM8108 (versión 8x8 MIMO, enterprise)
    \item \texttt{netifd-morse}: Integración con netifd para configuración UCI nativa
    \item \texttt{morse-utils}: Herramientas de línea de comandos para diagnóstico HaLow
    \item \texttt{morse-hwsim}: Simulador de hardware HaLow para testing sin módulo físico
\end{itemize}

Device tree overlays cargados en \texttt{/boot/distroconfig.txt}:
\begin{verbatim}
...
\textbf{Opción 1: Imagen pre-compilada Morse Micro (Recomendado)}:
\begin{verbatim}
...
\textbf{Opción 2: Build personalizado desde código fuente}:
Si compilaste el firmware en la sección anterior, la imagen estará en:
\begin{verbatim}
cd openwrt
...
\textbf{Opción 3: Imagen estándar OpenWRT (sin soporte HaLow nativo)}:
\begin{verbatim}
